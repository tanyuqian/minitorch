...
if backend_name == "CudaOps":
    BACKEND = minitorch.TensorBackend(minitorch.CudaOps)

elif backend_name == "CudaKernelOps":
...
class Network(minitorch.Module):
    ...
    2. Apply a Linear layer to size 32 followed by a ReLU and Dropout.
    ...
    def forward(self, embeddings):
        ...
        # ASSIGN4.5
        x = self.linear1(x.view(embeddings.shape[0], self.embedding_dim)).relu()
        
        x = minitorch.dropout(x, self.dropout, self.mode == "eval")
        
        logit = self.linear2(x)
        
        out = logit.sigmoid().view(logit.shape[0])
...
class SentenceSentimentTrain:
    ...
    def train(
        ...
        optim = minitorch.Adam(self.model.parameters(), lr=learning_rate)
