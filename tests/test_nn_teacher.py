...
def test_gelu_teacher(sizes, backend):
...
@pytest.mark.parametrize("sizes", GENERAL_SHAPES)
@pytest.mark.parametrize("backend", _BACKENDS, ids=["CudaKernelOps"])
def test_logsumexp_teacher(sizes, backend):
...
@pytest.mark.parametrize("backend", _BACKENDS, ids=["CudaKernelOps"])
def test_one_hot_teacher(batches, classes, backend):
...
@pytest.mark.parametrize("classes", [2, 32, 128, 10000])
@pytest.mark.parametrize("backend", _BACKENDS, ids=["CudaKernelOps"])
def test_softmax_loss_teacher(batches, classes, backend):
